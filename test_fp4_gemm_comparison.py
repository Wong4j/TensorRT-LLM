...
def create_test_data(m: int, n: int, k: int, device: str = "cuda") -> Dict[str, torch.Tensor]:
    ...
    # 使用离散的整数值，避免浮点精度问题
    act_fp4_data = torch.randint(-10, 11, (m, k_compressed), device=device).float() * 0.1  # 范围 [-1.0, 1.0]
    weight_data = torch.randint(-10, 11, (n, k_compressed), device=device).float() * 0.1   # 范围 [-1.0, 1.0]
    ...
    # 使用固定的缩放因子，确保数值稳定性
    act_sf_values = torch.full((m, scale_groups), e4m3_one, dtype=torch.uint8, device=device)
    weight_scale_values = torch.full((n, scale_groups), e4m3_one, dtype=torch.uint8, device=device)
