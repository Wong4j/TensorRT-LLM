...
# cuBLASLt FP4 GEMM 测试构建和运行脚本

set -e

echo "=== cuBLASLt FP4 GEMM 功能测试 ==="
...
# 检查可执行文件
if [ -f "test_cublaslt_fp4_gemm" ]; then
    echo "✓ 可执行文件创建成功"
else
    echo "✗ 可执行文件创建失败"
    exit 1
...
echo "构建目录: $BUILD_DIR"
echo "可执行文件: $BUILD_DIR/test_cublaslt_fp4_gemm"
echo ""
echo "如需重新运行测试，请执行:"
echo "cd $BUILD_DIR && ./test_cublaslt_fp4_gemm"
