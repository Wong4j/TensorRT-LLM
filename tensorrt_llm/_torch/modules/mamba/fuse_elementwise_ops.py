...
def _transpose_and_split_qkv_kernel(
    ...
    out_dim = tl.where(pid_out == 2, v_dim,
                       tl.where(pid_out == 1, k_dim, q_dim))
    src_col_offset = tl.where(pid_out == 0, 0,
                              tl.where(pid_out == 1, q_dim, q_dim + k_dim))
    ...
    prefill_indices = prefill_src_col[None, :].to(tl.int64) * num_prefill + seq_offsets[:, None].to(tl.int64)
    prefill_data = tl.load(prefill_t_ptr + prefill_indices,
                           mask=is_prefill[:, None] & dim_mask[None, :],
                           other=0.0)

    # Decode: read from decode_ptr[T_d, D] row-major
    decode_row = seq_offsets - num_prefill
    decode_indices = decode_row[:, None].to(tl.int64) * num_cols + (src_col_offset + dim_offsets[None, :]).to(tl.int64)
    decode_data = tl.load(decode_ptr + decode_indices,
                          mask=is_decode[:, None] & dim_mask[None, :],
                          other=0.0)
    ...
    out_ptr = tl.where(pid_out == 0, q_ptr,
                       tl.where(pid_out == 1, k_ptr, v_ptr))
...
def transpose_and_split_qkv(
    ...
    q_flat = torch.empty(total_seq, q_dim, dtype=prefill_t.dtype,
                         device=prefill_t.device)
    k_flat = torch.empty(total_seq, k_dim, dtype=prefill_t.dtype,
                         device=prefill_t.device)
    v_flat = torch.empty(total_seq, v_dim, dtype=prefill_t.dtype,
                         device=prefill_t.device)

    BLOCK_SEQ, BLOCK_DIM = 32, 128
    max_dim = max(q_dim, k_dim, v_dim)
    grid = (triton.cdiv(total_seq, BLOCK_SEQ),
            triton.cdiv(max_dim, BLOCK_DIM), 3)

    _transpose_and_split_qkv_kernel[grid](
        prefill_t, decode, q_flat, k_flat, v_flat,
        num_prefill, num_decode, num_cols,
        q_dim, k_dim, v_dim,
        BLOCK_SEQ, BLOCK_DIM,
    )
...
def _split_qkv_contiguous_kernel(
    ...
    src_indices = (seq_offsets[:, None].to(tl.int64) * src_stride_seq
